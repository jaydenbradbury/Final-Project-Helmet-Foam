...
ggplot(df_clean, aes(x = log(Time), y = Estatic_rel, color = freq)) +
  ...
  theme_minimal()
```
In @fig-freq-estaticRel, we can see that for lower frequencies, there is a higher relative static stiffness level recorded. This trend is the same across the different foam types. Frequency is something that we will consider in our final model. 
...
```{r ref.label=c('front-matter', 'tbl-variables', 'fig-Figure1', 'fig-chem-poro', 'fig-correlation', 'fig-freq-estaticRel', 'Model-pvalue', 'em-plot', 'fig-estatRel-time','fig-estatRel-logTime', 'diagplots' )}
#| echo: true
#| eval: false
