...
st_stiff_model <- lmer(Estatic_rel ~ Stress_kPa + log(Time):Porosity*ChemIndex + freq +  num_squish+  (1|SampleCode), data = df)

summary(st_stiff_model)

library(nlme)
m1 <- lme(Estatic_rel ~   Stress_kPa + log(Time):Porosity*ChemIndex +  freq + num_squish ,random=~1|SampleCode,data=df)
anova(m1)
```
